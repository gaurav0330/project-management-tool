...
const projectService = {
  createProject: async ({
    ...
    // ✅ UPDATED: Added optional githubRepo
    ...
    try {
      ...
      if (project.projectManager.toString() !== user.id) {
        return {
          success: false,
          message:
            "Access Denied: Only the Project Manager can assign team leads.",
        };
      }
      ...
      for (const lead of formattedTeamLeads) {
        await updateGroupsOnUserChange({
          projectId: project._id,
          userId: lead.teamLeadId,
          action: "add",
        });
      }
  ...
  assignTaskService: async ({
    ...
    try {
      ...
      const project = await Project.aggregate([
        ...
        {
          $match: {
            "teamLeads.teamLeadId": new mongoose.Types.ObjectId(assignedTo),
          },
        },
        {
          $project: {
      ...
      if (project.length === 0) {
        return {
          success: false,
          message: "Assigned user is not a Team Lead of this project.",
          task: null,
        };
      }
      ...
      if (!teamLead) {
        return {
          success: true,
          message:
            "Task assigned but team lead not found for email notification.",
          task: newTask,
        };
      }
  ...
  approveTaskCompletion: async (taskId, approved, remarks, user) => {
    ...
    if (!project || project.projectManager.toString() !== user.id) {
      throw new Error(
        "Unauthorized! Only the Project Manager can approve tasks."
      );
    }
    ...
    if (!approved) {
      return {
        success: false,
        message:
          "Task approval denied. Please provide modifications or rejection.",
        task,
      };
    }
  ...
  rejectTask: async (taskId, reason, user) => {
    ...
    if (!project || project.projectManager.toString() !== user.id) {
      throw new Error(
        "Unauthorized! Only the Project Manager can reject tasks."
      );
    }
    ...
    if (!teamMember) {
      return {
        success: true,
        message:
          "Task rejected, but team member not found for email notification.",
        task,
      };
    }
  ...
  requestTaskModifications: async (taskId, feedback, user) => {
    ...
    if (!project || project.projectManager.toString() !== user.id) {
      throw new Error(
        "Unauthorized! Only the Project Manager can request modifications."
      );
    }
    ...
    if (!teamMember) {
      return {
        success: true,
        message:
          "Modification requested, but team member not found for email notification.",
        task,
      };
    }
    ...
    try {
      ...
      if (!task.createdBy || !task.assignedTo) {
        throw new ApolloError(
          "Task data is incomplete. Cannot determine permissions.",
          "BAD_REQUEST"
        );
      }
