...
export default function TaskDetails({ task }) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 text-center">
        ...
        <p className="font-body text-txt-secondary-light dark:text-txt-secondary-dark max-w-sm">
          Select a task from the list to view its complete details and manage its status
        </p>
      </div>
    );
  }
  ...
  const getStatusConfig = (status) => {
    switch (status?.toLowerCase()) {
      ...
      case "to do":
        return {
          classes: "bg-warning/10 text-warning border-warning/30 dark:bg-warning/20 dark:text-warning dark:border-warning/40",
          icon: Clock,
      ...
      case "done":
        return {
          classes: "bg-success/10 text-success border-success/30 dark:bg-success/20 dark:text-success dark:border-success/40",
          icon: CheckCircle,
          bgClass: "bg-success/5 dark:bg-success/10"
        };
      case "rejected":
        return {
          classes: "bg-error/10 text-error border-error/30 dark:bg-error/20 dark:text-error dark:border-error/40",
          icon: XCircle,
          bgClass: "bg-error/5 dark:bg-error/10"
        };
      case "in progress":
        return {
          classes: "bg-brand-primary-50 text-brand-primary-700 border-brand-primary-200 dark:bg-brand-primary-900/20 dark:text-brand-primary-300 dark:border-brand-primary-800",
          icon: Target,
      ...
      case "pending approval":
        return {
          classes: "bg-brand-secondary-50 text-brand-secondary-700 border-brand-secondary-200 dark:bg-brand-secondary-900/20 dark:text-brand-secondary-300 dark:border-brand-secondary-800",
          icon: AlertCircle,
      ...
      default:
        return {
          classes: "bg-bg-accent-light dark:bg-bg-accent-dark text-txt-secondary-light dark:text-txt-secondary-dark border-bg-accent-light dark:border-bg-accent-dark",
          icon: FileText,
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
        <div>
          <h2 className="font-heading text-2xl font-bold text-heading-primary-light dark:text-heading-primary-dark mb-2 leading-tight">
            {task.title}
          </h2>
          <div className="flex items-center gap-3">
            ...
            {/* Priority Badge */}
            {task.priority && (
              <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm ${priorityConfig.classes} ${priorityConfig.bgClass}`}>
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        >
          ...
          {dueDateInfo.status === 'overdue' && (
            <span className="ml-auto text-xs bg-error/10 text-error px-2 py-1 rounded-full font-medium animate-pulse">
              OVERDUE
            </span>
          )}
          {dueDateInfo.status === 'urgent' && (
