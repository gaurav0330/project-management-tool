...
const Chat = ({ projectId }) => {
  ...
  const { data: groupsData, loading: groupsLoading, error: groupsError } = useQuery(getGroupsQuery(), {
    variables:
                    ...
                    <div key={msg.id} className={`w-full flex ${isSelf ? "justify-end" : "justify-start"}`}>
                      <div className={`relative group flex flex-col items-end max-w-[80%] md:max-w-xl ${isSelf ? "items-end" : "items-start"}`}>
                        <div className={`
                          ...
                          ${isSelf
                            ? "bg-gradient-to-l from-brand-primary-600 via-brand-primary-400/95 to-brand-primary-300/80 text-bg-primary-light"
