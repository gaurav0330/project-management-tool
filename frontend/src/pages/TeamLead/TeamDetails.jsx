...
import {
  ...
  FaArrowLeft, FaUserPlus, FaClipboardCheck
} from "react-icons/fa";
...
export default function TeamDetails() {
  ...
  const contentMarginLeft = isDesktop 
    ? (shouldShowFullSidebar ? 256 : 64)  // 16rem = 256px, 4rem = 64px
    : 0;
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-6">
                          ...
                          <div>
                            <motion.h1 className="font-heading text-4xl font-bold mb-2"
                              ...
                              {loading ? "Loading team..." : teamData?.teamName || "—"}
                            </motion.h1>
                            <motion.p className="font-body text-lg opacity-90"
                              initial={{ opacity: 0, x: -20 }}
                              animate={{ opacity: 1, x: 0 }}
                              transition={{ duration: 0.6, delay: 0.3 }}>
                              {loading ? "" : teamData?.description || ""}
                            </motion.p>
                          </div>
                        </div>
                        <motion.div className="grid grid-cols-2 gap-4"
                          ...
                          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 text-center">
                            <p className="font-heading text-2xl font-bold">{loading ? "—" : teamData?.memberCount ?? "—"}</p>
                            <p className="font-body text-sm opacity-80">Members</p>
                          </div>
                          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 text-center">
                            <p className="font-heading text-2xl font-bold">{loading ? "—" : teamData?.taskCount ?? "—"}</p>
