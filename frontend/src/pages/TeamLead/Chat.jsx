    ...
    <div className="flex h-screen bg-bg-primary-light dark:bg-bg-primary-dark text-txt-primary-light dark:text-txt-primary-dark font-body">
      ...
      <aside className="w-80 bg-bg-secondary-light dark:bg-bg-secondary-dark border-r border-gray-200 dark:border-gray-700 flex flex-col">
        <div className="p-5 border-b border-gray-200 dark:border-gray-700">
          <h1 className="text-xl font-heading font-bold text-heading-primary-light dark:text-heading-primary-dark">
            Team Lead Chat
          </h1>
        ...
        <div className="flex-1 overflow-y-auto">
          {groups.length === 0 ? (
            <div className="p-4 text-center text-txt-muted-light dark:text-txt-muted-dark">
              <p>No chat groups available</p>
              <p className="text-sm mt-1">Groups will appear when you're assigned to teams</p>
            </div>
          ) : (
            groups.map((group) => (
              <button
                ...
                className={`w-full text-left p-4 border-b border-gray-100 dark:border-gray-700 rounded-none hover:bg-bg-accent-light dark:hover:bg-bg-accent-dark hover:text-interactive-primary-light dark:hover:text-interactive-primary-dark transition-colors focus:outline-none ${
                  selectedGroup?.id === group.id
                    ? "bg-bg-accent-light border-l-4 border-brand-accent-500 dark:bg-bg-accent-dark dark:border-brand-accent-400 text-brand-accent-600 dark:text-brand-accent-400 font-semibold"
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-xs text-txt-muted-light dark:text-txt-muted-dark uppercase ml-2">
                    {group.type}
                  </div>
                </div>
              </button>
        ...
        {selectedGroup ? (
          <>
            ...
            <header className="bg-bg-secondary-light dark:bg-bg-secondary-dark border-b border-gray-200 dark:border-gray-700 p-5">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-lg font-heading font-semibold text-heading-primary-light dark:text-heading-primary-dark">{selectedGroup.name}</h2>
                ...
                <div className="text-xs text-txt-muted-light dark:text-txt-muted-dark">
            ...
            <section className="flex-1 overflow-y-auto p-6 space-y-5">
              {messagesLoading ? (
                <div className="flex items-center justify-center h-32">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-4 border-brand-accent-500 dark:border-brand-accent-400"></div>
                </div>
                    ...
                    >
                      <div
                        className={`max-w-xs lg:max-w-lg px-5 py-3 rounded-lg whitespace-pre-wrap break-words select-text ${
                          isCurrentUser
                            ? "bg-brand-accent-600 text-bg-primary-light font-body"
