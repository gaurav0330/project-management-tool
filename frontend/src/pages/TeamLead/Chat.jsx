...
const TeamLeadChat = ({ projectId }) => {
  ...
  const { data: groupsData, loading: groupsLoading, error: groupsError } = useQuery(
    GET_GROUPS_FOR_LEAD,
    {
      variables: { leadId: currentUser?.id, projectId },
  ...
  const groups = groupsData?.getGroupsForLead || [];
  ...
  return (
    <div className="flex h-screen bg-bg-primary-light dark:bg-bg-primary-dark font-body">
      {/* Sidebar */}
      <aside
        className="min-w-[20rem] max-w-xs w-80 flex flex-col shadow-2xl rounded-r-3xl
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-2">
          {groups.length === 0 ? (
            <div className="p-8 text-center text-txt-muted-light dark:text-txt-muted-dark">
              <p>No chat groups</p>
              <p className="text-sm mt-2">Groups appear when you’re assigned as lead</p>
            </div>
          ) : (
            groups.map((group) => (
              <button
                ...
                className={`
                  group w-full mb-2 last:mb-0 flex items-center gap-3 px-4 py-3 rounded-2xl
                  hover:bg-brand-accent-100/60 dark:hover:bg-brand-accent-700/60 hover:shadow-lg
                  border-2
                  transition-all duration-150
                  ${
                    selectedGroup?.id === group.id
                      ? "border-brand-accent-600 dark:border-brand-accent-400 bg-brand-accent-50/80 dark:bg-brand-accent-900 shadow"
                      : "border-transparent bg-transparent"
                  }
                  focus:outline-none focus:ring-2 focus:ring-brand-accent-500
                `}
              >
                {/* Avatar Initials */}
                <span
                  className={`
                    font-heading font-extrabold text-lg rounded-full
                    w-10 h-10 flex items-center justify-center
                    ${selectedGroup?.id === group.id
                      ? "bg-brand-accent-600 text-bg-primary-light shadow"
        ...
        {selectedGroup ? (
            ...
            <header className="sticky top-0 z-10 flex items-center gap-3 bg-bg-secondary-light/80 dark:bg-bg-secondary-dark/90 backdrop-blur-lg border-b border-brand-primary-100 dark:border-brand-primary-900 px-10 py-6">
              ...
              <div className="flex items-center gap-2 flex-wrap max-w-[300px]">
                {selectedGroup.members?.slice(0, 5).map((m) => (
                  <span
                    key={m.id}
                    className="bg-brand-primary-100 dark:bg-brand-primary-900 text-brand-primary-700 dark:text-brand-primary-200 rounded px-2 py-0.5 text-xs font-mono font-semibold"
                  >{m.username}</span>
                ))}
            ...
            <section className="flex-1 overflow-y-auto px-6 md:px-14 py-8 space-y-6 bg-transparent scrollbar-thin">
              {messagesLoading ? (
                <div className="flex items-center justify-center h-40">
                  <div className="animate-spin rounded-full h-10 w-10 border-[3px] border-brand-accent-300 dark:border-brand-accent-600 border-b-transparent"></div>
                </div>
                ...
                messagesData?.getMessages?.map((msg, idx) => {
                  ...
                  return (
                    <div
                      key={msg.id}
                      className={`w-full flex ${isSelf ? "justify-end" : "justify-start"}`}>
                      <div className={`
                        relative group flex flex-col items-end max-w-[80%] md:max-w-xl
                      ...
                      `}>
                        <div className={`
                          ...
                          ${isSelf
                            ? "bg-gradient-to-l from-brand-accent-600 via-brand-accent-400/95 to-brand-accent-300/80 text-bg-primary-light"
                        ...
                        `}>
                          ...
                          {isTail && (
                            <span className={`absolute ${isSelf ? "bottom-0 -right-3" : "bottom-0 -left-3"} w-4 h-4 overflow-hidden`}>
                              <svg width="16" height="16" className={`${isSelf ? "text-brand-accent-600" : "text-bg-accent-light dark:text-bg-accent-dark"}`} viewBox="0 0 16 16" fill="currentColor">
            ...
            <footer className="sticky bottom-0 z-10 bg-bg-secondary-light/90 dark:bg-bg-secondary-dark/95 backdrop-blur-xl border-t border-brand-primary-100 dark:border-brand-primary-900 px-10 py-5">
              <form onSubmit={handleSendMessage} className="flex items-center gap-4">
                <input
                  ...
                  className="flex-1 px-6 py-3 rounded-full border border-brand-accent-100 dark:border-brand-accent-900 bg-bg-primary-light/90 dark:bg-bg-primary-dark/80 text-txt-primary-light dark:text-txt-primary-dark placeholder-txt-muted-light dark:placeholder-txt-muted-dark focus:outline-none focus:ring-2 focus:ring-brand-accent-300 dark:focus:ring-brand-accent-500 focus:ring-offset-1 shadow"
          ...
          <div className="flex-1 flex items-center justify-center px-8 bg-transparent">
            <div className="bg-bg-secondary-light/70 dark:bg-bg-secondary-dark/80 rounded-3xl p-12 shadow-2xl text-center text-txt-muted-light dark:text-txt-muted-dark w-full max-w-lg mx-auto">
              <svg width="40" height="40" className="mx-auto mb-3 text-brand-accent-400 dark:text-brand-accent-600 opacity-50" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="1.5"/><path d="M7 13h10M7 9h7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>
